...
export async function POST(request: NextRequest) {
  try {
    // 한국어 페이지 데이터 파일 경로
    const dataFilePath = join(process.cwd(), 'data', 'page-content.json');
    
    // 한국어 페이지 이미지 디렉토리 경로
    const imageDir = join(process.cwd(), 'public', 'uploads', 'pages', 'ko');
    const contentImageDir = join(process.cwd(), 'public', 'uploads', 'ko');
    ...
    if (existsSync(dataFilePath)) {
      try {
        ...
        console.error('Error deleting Korean page data:', error);
    ...
    if (existsSync(imageDir)) {
      try {
        ...
        console.error('Error reading Korean page images directory:', error);
    ...
    if (existsSync(contentImageDir)) {
      try {
        ...
        console.error('Error reading Korean content images directory:', error);
    ...
    const message = success 
      ? `한국어 페이지 데이터와 ${result.pageImagesDeleted}개의 페이지 이미지, ${result.contentImagesDeleted}개의 콘텐츠 이미지가 삭제되었습니다.`
    ...
    console.error('Error deleting Korean page data and images:', error);
    return NextResponse.json({ 
      success: false, 
      message: '한국어 페이지 데이터와 이미지를 삭제하는 중 오류가 발생했습니다.',
